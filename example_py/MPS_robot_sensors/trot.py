...
class TrotPolicy:
    def __init__(self, config, policy_name, freq=50):

        input_dim = 45
        action_dim = 12
        self.device = config['networks']['device']
        self.scaling_factors = config['scaling_' + policy_name]
        self.scale = config['scaling_' + policy_name]['q_des']
        self.q_def = config['robot'][policy_name]['default_joint_angles']
        ...
        policy_path = ''
        self.locosim = config['settings']['tests']['locosim']
        if self.locosim:
            policy_path += os.environ["LOCOSIM_DIR"] + '/robot_control/AlienGo_SDK/example_py/'
        policy_path += config['networks']['paths']['trot' + policy_name[1:]]
        state_dict = \
        torch.load(policy_path, map_location=torch.device(config['networks']['device']), weights_only=False)['model']
        ...
        # self.commands = np.array([-0.3, -0.06, -0.08])
        # self.commands = np.array([1., 0.0, 0.0])
    ...
    def compute_actions(self, imu_quat, imu_gyro, qpos, qvel):
        ...
        if self.decimation_counter == 0:
            ...
            obs = np.concatenate((scaled_body_vel, scaled_commands, scaled_gravity_body, scaled_joint_angles,
