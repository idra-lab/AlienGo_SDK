...
def computeBackup(q_data, v_data, backup_nn, imu, last_action):
    """ 
...
class MPS:
    def __init__(self, decimation, max_pos, min_pos, torque_values, Kp_n, Kd_n, config_b):
        ...
        dt = 0.01 # Time step to update desired joint angles
    ...
    def isRecSingle(self, qDes):
        ...
        for i in range(0, self.N_mps): ##simulated steps
            ...
            pos_backup_order, last_action = computeBackup(q_muj[7:], v_muj[6:], self.backup_nn, imu, last_action)
            iter_mps += 1
