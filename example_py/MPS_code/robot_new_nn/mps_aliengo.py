...
from utils import scale_axis, quat_rotate_inverse, swap_legs, clip_torques_in_groups, orderState, orderBackup
import pygame
...
config_path = "/home/aliengo_ws/Documents/projects/AlienGo_SDK/example_py/MPS_code/robot_new_nn/config_mps.yaml"
...
################
joint_def_nn = config['scaling']['default_joint_angles']
...
def compute_observation(state, scaling_factors):
    ...
    return np.concatenate((vel_comm, body_acc, pos_order, vel_order, latest_actions))
...
def compute_actions(state, scaling_factors):
    ...
    while not stop_threads:
        ...
        if use_backup:
            ...
            new_actions = orderBackup(pos_backup)#####swap_legs(new_actions1)
        else:
            new_actions1 = pos_mpc[iter_mpc]
            new_actions = (orderState(new_actions1) - joint_def_nn)/ 0.8
        ...
        if motiontime >= 1*(1/dt):
            ...
            if use_backup:
                ...
                for key in d:
                    ...
                    k += 1
                latest_actions_scaled = orderBackup((latest_actions * 0.8) + joint_def_nn)
                ctrl = Kd_nn * (- dq_real) + Kp_nn * (latest_actions_scaled - q_real)
                u = np.clip(ctrl, low_tau, high_tau)
            print(qDes)
            print(u)
