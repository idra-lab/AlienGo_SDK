...
def compute_observation(data, scaling_factors):
    """
    ...
    """
    commands = np.array([0,0,0])#get_commands() # The stopping condition here is not evaluated
...
if __name__ == '__main__':
    ...
    desc_dir = 'MPS_code/models/aliengo_models'
    xml = desc_dir + '/xml/aliengo.xml'
    spec = mujoco.MjSpec()
    spec.from_file(xml)
    model_muj = spec.compile()
    data_muj = mujoco.MjData(model_muj)
    data_muj.qpos = np.array([ 2.35160918e-02,  2.99307019e-03,  5.69021180e-02,  6.78199462e-04,
                              -2.45632626e-05, -1.76856369e-05,  9.99999770e-01, -6.95906523e-01,
                               1.12774864e+00, -2.69598249e+00,  6.73169302e-01,  1.14516356e+00,
                              -2.71308710e+00, -6.57673870e-01,  1.15622358e+00, -2.72399097e+00,
                               6.88303601e-01,  1.13360258e+00, -2.70162319e+00])
    ...
    threading.Thread(target=compute_actions, args=(data_muj, scaling_factors), daemon=True).start()

    with mujoco.viewer.launch_passive(model_muj, data_muj) as viewer:
        data_muj.qacc_warmstart = 0
        while motiontime <= 20000:#True:
            """
            ...
            if( motiontime >= 0 and motiontime < 1*(1/dt)):
                ...
                qInit = data_muj.qpos[7:]#[state.motorState[d[key]].q for key in d]
           ...
           # print('qDes',qDes)
            if motiontime >= 1*(1/dt):
                ...
                u = Kd * (- data_muj.qvel[6:]) + Kp * (qDes - data_muj.qpos[7:]) + torque_values
                data_muj.ctrl = np.clip(u, -40, 40)
                mujoco.mj_step(model_muj, data_muj)
                renderer.update_scene(data_muj)
                viewer.sync()
           # print('data_muj.qpos[7:]',data_muj.qpos[7:])
