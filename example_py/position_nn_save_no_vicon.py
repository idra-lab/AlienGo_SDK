...
def get_stop_walk_button():
    stop_walk_button = joystick.get_button(2)
    if stop_walk_button:
        return True
    else:
        return False
...
def save_data(state,motiontime,current_timestamp):
    ...
    joint_tau = [state.motorState[i].tauEst for i in range(12)]
    foot_force = [state.footForce[i] for i in range(4)]
    foot_force_est = [state.footForceEst[i] for i in range(4)]

    save = []
    save.append(current_timestamp)
    save.append(state.tick)
    save.append(motiontime)
    save.extend(body_acc)
    save.extend(body_vel)
    save.extend(joint_angles)
    save.extend(joint_velocities)
    save.extend(joint_tau)
    save.extend(foot_force)
    save.extend(foot_force_est)

    data_robot.append(save)
    #print(data_robot)
  ...
  #  streamer.start()
    ...
    while True:
        ...
        if check_safety_stops(state):  # Using qpos to check inclination
            ...
            with open(name_save, 'a', encoding="ISO-8859-1", newline='') as myfile:
                wr = csv.writer(myfile)
                wr.writerows(data_robot)
            myfile.close()
            exit()

        if get_stop_walk_button():
        ...
        elif( motiontime >= 1*(1/dt) and motiontime < 7*(1/dt)) or stop_walk:
            rate_count += 1
